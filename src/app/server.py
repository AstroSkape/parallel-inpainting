...
def run_inpaint(image_path: str) -> tuple:
    ...
    for line in result.stdout.splitlines():
        if line.startswith("METRICS_JSON:"):
            metrics.update(json.loads(line[len("METRICS_JSON:"):]))
        if line.startswith("TIMING_JSON:"):
            metrics.update(json.loads(line[len("TIMING_JSON:"):]))

    print("Parsed metrics:", metrics)
    print("CPU out exists:", os.path.exists(cpu_out))
    print("GPU out exists:", os.path.exists(gpu_out))
...
def inpaint():
    ...
    try:
        ...
        return jsonify({"error": f"Inpaint run failed: {e}"}), 500
    ...
    response = {
        "cpu_time":   round(cpu_time, 3) if cpu_time else None,
