...
void NearestNeighborField::minimize(int nr_pass, bool is_gpu_enabled, CudaNNFDeviceBuffers *cuda_bufs, int *d_field_ptr) {
	const auto &this_size = source_size();

	if (is_gpu_enabled) {
		minimize_cuda(nr_pass, cuda_bufs, d_field_ptr);
		return;
	}
...
int DebugPatchSSDDistanceMetric::operator()(const MaskedImage &source,
...
int RegularityGuidedPatchDistanceMetricV1::operator()(
	const MaskedImage &source, int source_y, int source_x,
	const MaskedImage &target, int target_y, int target_x) const {
	double dx =
		remainder(double(source_x - target_x) / source.size().width, m_dx1);
	double dy =
		remainder(double(source_y - target_y) / source.size().height, m_dy2);
	...
	double score2 = distance_masked_images(source, source_y, source_x, target,
										   target_y, target_x, m_patch_size);
...
int RegularityGuidedPatchDistanceMetricV2::operator()(
	const MaskedImage &source, int source_y, int source_x,
	const MaskedImage &target, int target_y, int target_x) const {
	if (target_y < 0 || target_y >= target.size().height || target_x < 0 ||
		target_x >= target.size().width)
		return PatchDistanceMetric::kDistanceScale;
	...
	// source_y * source_scale, m_ijmap.size().height, source_x * source_scale,
	// m_ijmap.size().width);

	double score1 = PatchDistanceMetric::kDistanceScale;
	if (!source.is_globally_masked(source_y, source_x) &&
		!target.is_globally_masked(target_y, target_x)) {
		auto source_ij = m_ijmap.ptr<float>(source_y * source_scale,
											source_x * source_scale);
		auto target_ij = m_ijmap.ptr<float>(target_y * target_scale,
											target_x * target_scale);
