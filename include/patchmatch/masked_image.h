...
class MaskedImage {
  public:
	MaskedImage()
		: m_image(), m_mask(), m_global_mask(), m_image_grady(),
		  m_image_gradx(), m_image_grad_computed(false) {}
	MaskedImage(cv::Mat image, cv::Mat mask)
		: m_image(image), m_mask(mask), m_image_grad_computed(false) {}
	MaskedImage(cv::Mat image, cv::Mat mask, cv::Mat global_mask)
		: m_image(image), m_mask(mask), m_global_mask(global_mask),
		  m_image_grad_computed(false) {}
	MaskedImage(cv::Mat image, cv::Mat mask, cv::Mat global_mask, cv::Mat grady,
				cv::Mat gradx, bool grad_computed)
		: m_image(image), m_mask(mask), m_global_mask(global_mask),
		  m_image_grady(grady), m_image_gradx(gradx),
	...
	inline MaskedImage clone() {
		return MaskedImage(m_image.clone(), m_mask.clone(),
						   m_global_mask.clone(), m_image_grady.clone(),
						   m_image_gradx.clone(), m_image_grad_computed);
	}
	...
	inline bool is_globally_masked(int y, int x) const {
		return !m_global_mask.empty() &&
			   static_cast<bool>(m_global_mask.at<unsigned char>(y, x));
	}
	...
	inline void set_global_mask(int y, int x, bool value) {
		m_global_mask.at<unsigned char>(y, x) =
	...
	MaskedImage upsample(int new_w, int new_h,
						 const cv::Mat &new_global_mask) const;
