...
bool MaskedImage::contains_mask(int y, int x, int patch_size) const {
	...
	for (int dy = -patch_size; dy <= patch_size; ++dy) {
		for (int dx = -patch_size; dx <= patch_size; ++dx) {
			int yy = y + dy, xx = x + dx;
			if (yy >= 0 && yy < mask_size.height && xx >= 0 &&
				xx < mask_size.width) {
...
MaskedImage MaskedImage::downsample() const {
	...
	for (int y = 0; y < size.height - 1; y += 2) {
		for (int x = 0; x < size.width - 1; x += 2) {
			...
			for (int dy = -kernel_size.height / 2 + 1;
				 dy <= kernel_size.height / 2; ++dy) {
				for (int dx = -kernel_size.width / 2 + 1;
					 dx <= kernel_size.width / 2; ++dx) {
					int yy = y + dy, xx = x + dx;
					if (yy >= 0 && yy < size.height && xx >= 0 &&
						xx < size.width) {
						...
						if (!is_masked(yy, xx)) {
							...
							int k = kernel[kernel_size.height / 2 - 1 + dy] *
									kernel[kernel_size.width / 2 - 1 + dx];
							r += source_ptr[0] * k, g += source_ptr[1] * k,
								b += source_ptr[2] * k;
							ksum += k;
						}
					}
...
MaskedImage MaskedImage::upsample(int new_w, int new_h,
